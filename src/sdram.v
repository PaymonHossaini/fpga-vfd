// Simple SDRAM controller for Tang Nano 20K
// Based on nand2mario's controller from NESTang
// 
...
// - Read/write operations take 5 cycles to complete.
// - All reads/writes are done with auto-precharge.
// - SDRAMs need periodic refreshes. Call refresh at least once every ~15us.
//
// Requires a 180-degree phase-shifted clock (clk_sdram) for SDRAM.
...
module sdram #(
    ...
    input             refresh,          // command: auto refresh
    input      [22:0] addr,             // byte address
    input       [7:0] din,              // data input
    output      [7:0] dout,             // data output
    output reg        data_ready,       // read data valid
...
always @(posedge clk) begin
    ...
    casex ({state, cycle})
        ...
        {READ, T_RCD}: begin
            ...
            SDRAM_A[9:0] <= {1'b0, addr[COL_WIDTH-1+2:2]};
            SDRAM_DQM <= 4'b0;
            off <= addr[1:0];
        end
...
//
// Generate cfg_now pulse after initialization delay (200us)
//
reg  [14:0]   rst_cnt;
