// GU-7000 Simple ASCII Test
// Just sends "ABCD" (0x41, 0x42, 0x43, 0x44) as direct character codes
// This helps verify bit ordering

module vfd_7000_ascii_test (
    ...
    localparam WR_LOW_CYCLES = 8;        // ~300ns
    localparam POST_DELAY_CYCLES = 600;  // ~22us
    ...
    // Just send "ABCD" - direct ASCII characters
    wire [7:0] cmd_data [0:3];
    assign cmd_data[0] = 8'h41;  // 'A'
    assign cmd_data[1] = 8'h42;  // 'B'
    assign cmd_data[2] = 8'h43;  // 'C'
    assign cmd_data[3] = 8'h44;  // 'D'
    ...
    always @(posedge clk) begin
        case (state)
            STATE_INIT: begin
                counter <= counter + 1;
                if (counter >= 24'd2_700_000) begin  // 100ms startup
                    counter <= 0;
                    byte_index <= 0;
