// VFD Counter Pattern Test - outputs changing byte pattern (no ROM)
// Each byte increments: 0x00, 0x01, 0x02... wrapping every 256 bytes
// This tests if changing data values cause corruption
...
module vfd_image_display (
    ...
    localparam STATE_DMA_SETUP  = 4'd8;
    localparam STATE_DMA_PULSE  = 4'd9;
    localparam STATE_DMA_HOLD   = 4'd10;
    localparam STATE_DMA_WAIT   = 4'd11;
    localparam STATE_DMA_NEXT   = 4'd12;
    ...
    localparam DMA_SETUP = 16'd27;     // ~1us data setup
    localparam DMA_PULSE = 16'd27;     // ~1us WR low pulse
    localparam DMA_HOLD  = 16'd27;     // ~1us data hold
    ...
    reg [12:0] data_idx;  // Counter for data bytes
        ...
        case (state)
            ...
            STATE_HDR_NEXT: begin
                ...
                if (byte_idx < NUM_HEADER - 1) begin
                    ...
                    byte_idx <= NUM_HEADER;
                    data_idx <= 13'd0;
                    latched_byte <= 8'h00;  // First byte is 0x00
                    state <= STATE_DMA_SETUP;
                end
            end
            
            // ========== DMA DATA (counter pattern, no ROM) ==========
            ...
            STATE_DMA_NEXT: begin
                ...
                if (data_idx < NUM_DATA - 1) begin
                    data_idx <= data_idx + 1;
                    byte_idx <= byte_idx + 1;
                    // Next byte is lower 8 bits of data_idx+1 (counter pattern)
                    latched_byte <= data_idx[7:0] + 8'd1;
                    state <= STATE_DMA_SETUP;
                end else begin
                    state <= STATE_DONE;
                end
