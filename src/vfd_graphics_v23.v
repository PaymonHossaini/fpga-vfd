// VFD Graphics Test v23 - Better timing with delays between commands
// Add proper delays after clear and before graphics command

module vfd_graphics_v23 (
    input wire clk,
    ...
    // Which bytes need extra delay after them
    wire needs_long_delay = (byte_idx == 6'd0);  // After clear command
    ...
    always @(posedge clk) begin
        case (state)
            ...
            STATE_WAIT_RDY: begin
                ...
                if (ready || counter > 24'd270000) begin  // ~10ms timeout
                    state <= STATE_SETUP;
                    counter <= 24'd0;
            ...
            STATE_PULSE_WR: begin
                ...
                leds <= 6'b001000;
                // Longer write pulse
                if (wr_timer < 8'd50) wr_timer <= wr_timer + 1;
                else begin wr_timer <= 8'd0; state <= STATE_HOLD; end
            end
            STATE_HOLD: begin
                wr_n <= 1'b1;
                leds <= 6'b010000;
                // Long delay after clear command (50ms), shorter for others
                if (needs_long_delay) begin
                    if (counter < 24'd1350000) counter <= counter + 1;  // 50ms
                    else begin counter <= 24'd0; state <= STATE_NEXT; end
                end else begin
                    if (wr_timer < 8'd100) wr_timer <= wr_timer + 1;
