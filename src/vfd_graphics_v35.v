// VFD Graphics Test v35 - Fixed timing with data setup delay
// Timing: data setup -> WR low -> WR high -> hold delay

module vfd_graphics_v35 (
    input wire clk,
    ...
    localparam STATE_SETUP      = 4'd2;  // Put data on bus
    localparam STATE_SETUP_HOLD = 4'd3;  // NEW: Wait for data to be stable
    localparam STATE_PULSE_WR   = 4'd4;  // WR goes low
    localparam STATE_HOLD       = 4'd5;  // WR goes high, hold data
    localparam STATE_POST_DELAY = 4'd6;  // Wait between bytes
    ...
    // Simple test: just dots like v32 which worked
    // 64 nulls + Init(2) + Clear(1) + 5 dots × 9 bytes = 112
    ...
    always @(*) begin
        if (byte_idx < NUM_NULLS) begin
            ...
            case (byte_idx - NUM_NULLS)
                ...
                8'd17: begin current_byte = 8'h14; post_delay = DELAY_SHORT; end
                ...
                8'd35: begin current_byte = 8'h0A; post_delay = DELAY_SHORT; end
                8'd36: begin current_byte = 8'h00; post_delay = DELAY_SHORT; end
                8'd37: begin current_byte = 8'h14; post_delay = DELAY_SHORT; end
                ...
                8'd44: begin current_byte = 8'h0A; post_delay = DELAY_SHORT; end
                8'd45: begin current_byte = 8'h00; post_delay = DELAY_SHORT; end
                8'd46: begin current_byte = 8'h1E; post_delay = DELAY_SHORT; end
