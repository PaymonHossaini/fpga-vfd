// VFD Graphics Test v38 - Bit image with exact byte count
// No extra bytes after image data

module vfd_graphics_v38 (
    input wire clk,
    ...
    // Exact count: 64 nulls + 2 init + 1 clear + 13 cmd + 32 data = 112
    // But actually: cmd is bytes 3-15 (13 bytes), data is 16-47 (32 bytes)
    // So last byte is index 47, which is byte 48 total after nulls
    // 64 + 48 = 112 total, indices 0-111
    localparam NUM_BYTES = 8'd112;  // indices 0 to 111
    ...
    reg [23:0] post_delay;
    reg valid_byte;  // Flag to indicate if this is a real byte to send
    ...
    always @(*) begin
        ...
        if (byte_idx < NUM_NULLS) begin
            ...
            case (byte_idx - NUM_NULLS)
                ...
                // Dot unit real-time bit image: US ( d 21h
                ...
                // Position: (10, 10) - offset from corner to see it clearly
                8'd7:  begin current_byte = 8'h0A; post_delay = DELAY_SHORT; end  // xPL = 10
                8'd8:  begin current_byte = 8'h00; post_delay = DELAY_SHORT; end  // xPH
                8'd9:  begin current_byte = 8'h0A; post_delay = DELAY_SHORT; end  // yPL = 10
                8'd10: begin current_byte = 8'h00; post_delay = DELAY_SHORT; end  // yPH
                
                // Size: 16x16 pixels
                8'd11: begin current_byte = 8'h10; post_delay = DELAY_SHORT; end  // xL = 16
                8'd12: begin current_byte = 8'h00; post_delay = DELAY_SHORT; end  // xH
                8'd13: begin current_byte = 8'h10; post_delay = DELAY_SHORT; end  // yL = 16
                8'd14: begin current_byte = 8'h00; post_delay = DELAY_SHORT; end  // yH
                ...
                8'd42: begin current_byte = 8'h01; post_delay = DELAY_SHORT; end
                8'd43: begin current_byte = 8'h80; post_delay = DELAY_SHORT; end
                8'd44: begin current_byte = 8'h01; post_delay = DELAY_SHORT; end
                8'd45: begin current_byte = 8'h80; post_delay = DELAY_SHORT; end
                // Column 15: right edge
                8'd46: begin current_byte = 8'hFF; post_delay = DELAY_SHORT; end
                8'd47: begin current_byte = 8'hFF; post_delay = DELAY_LONG; end
                
                default: begin 
                    current_byte = 8'h00; 
                    post_delay = DELAY_SHORT; 
                    valid_byte = 1'b0;  // Don't send anything after byte 47
                end
            endcase
        end
    end
    ...
    always @(posedge clk) begin
        case (state)
            ...
            STATE_NEXT: begin
                // Stop after byte 47 (64 nulls + 48 data bytes - 1 = index 111)
                if (byte_idx < 8'd111) begin
                    byte_idx <= byte_idx + 1;
