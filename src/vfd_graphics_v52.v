// VFD Graphics Test v52 - Use ROM instead of case statement
// Test if large case statement is causing synthesis issues

module vfd_graphics_v52 (
    input wire clk,
    ...
    // ROM for command bytes (registered, not combinational)
    ...
    // Register the ROM output
    always @(posedge clk) begin
        if (byte_idx < NUM_NULLS) begin
            current_byte <= 8'h00;
        end else begin
            current_byte <= rom_data[byte_idx - NUM_NULLS];
        end
    end
    
    // Determine if long delay needed
    always @(*) begin
        if (byte_idx < NUM_NULLS) begin
            is_long_delay = 0;
        end else begin
            case (byte_idx - NUM_NULLS)
                8'd1, 8'd2, 8'd11, 8'd20, 8'd29, 8'd38: is_long_delay = 1;
                default: is_long_delay = 0;
            endcase
        end
    end
        ...
        case (state)
            ...
            STATE_HOLD: begin
                ...
                if (delay_counter > 0)
                    ...
                    delay_counter <= is_long_delay ? DELAY_LONG : DELAY_SHORT;
                end
            end
