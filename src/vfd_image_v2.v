// VFD Image Display v2
// Confirmed: Data latches on RISING edge of WR
// Sequence: WR LOW -> data on bus -> wait -> WR HIGH (latch) -> wait
...
module vfd_image_v2 (
    ...
    // DMA states (rising edge latch)
    localparam STATE_DMA_FETCH  = 4'd8;
    localparam STATE_DMA_ROMWAIT= 4'd9;   // Wait for ROM data
    localparam STATE_DMA_WR_LOW = 4'd10;  // WR goes LOW
    localparam STATE_DMA_DATA   = 4'd11;  // Put data on bus
    localparam STATE_DMA_WR_HIGH= 4'd12;  // WR goes HIGH (latch)
    localparam STATE_DMA_WAIT   = 4'd13;  // Wait before next
    localparam STATE_DMA_NEXT   = 4'd14;
    ...
    // DMA timing - extended for debugging
    localparam DMA_WR_LOW_TIME = 16'd3;  // ~111ns WR low before data
    localparam DMA_SETTLE = 16'd5;       // ~185ns for data to settle (was 3)
    localparam DMA_WR_HIGH_TIME = 16'd3; // ~111ns hold after latch
    localparam DMA_WAIT   = 16'd405;     // ~15us between bytes
    
    localparam NUM_HEADER = 13'd8;
    localparam NUM_DATA = 13'd4096;
    
    // ROM - using hex-initialized block RAM
    ...
    always @(posedge clk) begin
        case (state)
            ...
            // ========== DMA (rising edge latch) ==========
            // Sequence: FETCH -> ROMWAIT -> WR_LOW -> DATA -> WR_HIGH -> WAIT -> NEXT
            
            STATE_DMA_FETCH: begin
                // ROM address is already set by byte_idx
                // Wait one cycle for ROM to output data
                wr_n <= 1'b1;
                data_bus <= 8'h00;
                timer <= 16'd0;
                leds <= 6'b100000;
                state <= STATE_DMA_ROMWAIT;
            end
            
            STATE_DMA_ROMWAIT: begin
                // ROM data is now valid, latch it
            ...
            STATE_DMA_WR_LOW: begin
                // WR goes LOW, wait before putting data
                wr_n <= 1'b0;
                data_bus <= 8'h00;
                if (timer < DMA_WR_LOW_TIME) begin
                    timer <= timer + 1;
                end else begin
                    timer <= 16'd0;
            ...
            STATE_DMA_DATA: begin
                // Put data on bus, WR still LOW
                // Wait for data to settle before rising edge
                data_bus <= latched_byte;
                wr_n <= 1'b0;
            ...
            STATE_DMA_WR_HIGH: begin
                // WR goes HIGH - DATA LATCHES HERE
                // Hold data for a bit
                data_bus <= latched_byte;
                wr_n <= 1'b1;
                if (timer < DMA_WR_HIGH_TIME) begin
                    timer <= timer + 1;
                end else begin
                    timer <= 16'd0;
